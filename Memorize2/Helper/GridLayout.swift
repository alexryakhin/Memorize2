...
struct GridLayout {
    //its very important to make this three vars private(set). If we don't make them private(set), someone just could think that they are able to reset our vars, but this vars are so important for our calculation so we definitely want this vars to be private(set)
